...
4. **早前里程碑清单在本仓库的失配是预期行为**（冻结时刻证据 + 同树生长）：`V0_M2.sha256` 现为 192 OK / 46 FAILED
   ...
   **不是**交付面被改。
5. **M3 后置修复（`ISS-20260923-001`）—— 落盘后独立全新克隆复验才暴露**：
   PM 从 `ca13fc0` 克隆到 **非 `/Users` 路径**（`/private/tmp/...`）复跑，得 **1 failed / 150 passed**；
   同一份交付物在 `/Users/wooyinq/...` 原位却是 **151 passed / 0 skipped** ⇒ **结果取决于检出目录前缀**。
   根因：`tests/test_calibrate_latency.py` 把 **`/Users/` 前缀**当成「绝对路径」的判据 ——
   夹具前置断言（L284）在非 `/Users` 检出下自己先失败；三条负向断言（L258/291/302）**全部空转**（假绿）。
   被测工具本身干净（`Path.is_absolute()` 判定，位置无关）。**性质与 M1 `f5f7762` 同类：判据绑死本机前缀。**
   **修法**：改为**位置无关**的「路径型取值是否以 `/` 开头」（`_absolute_path_values()`），
   比原来更强（任何绝对路径都抓）；负控已复验（helper 恒返回空 ⇒ 夹具前置断言变红）。
   **验证（PM 亲跑）**：仓库原位 13 passed；非 `/Users` 检出 13 passed；修复后非 `/Users` 检出全量
   **151 passed / 0 skipped**；`verify_specs` 130 checks OK；端到端基线逐位等于 M1。
   **清单口径**：按 M1 先例**保持冻结真相、不重取** ⇒ `shasum -c V0_M3.sha256` 现为 **201 OK / 1 FAILED**
   （`02_source` 149/150；失配行 `test_calibrate_latency.py` `466db840…` → `1a69ee09…`），**属预期行为**。
   **诚实边界**：这是**对已验收交付面的后置修改**，未经小队独立门禁；PM 自验 + 负控 + 非本机检出复跑为其证据。
