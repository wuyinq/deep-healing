...
| 边界 | 约束 |
|---|---|
| IP | 只借鉴《我的治愈系游戏》的设定**结构**与情绪内核（小区单元、自主住户、分级介入、治愈内核）；**禁止**复制原文/章节/台词，禁止搬用书中具体人物姓名与情节；对外命名与素材须可商用/自研 |
...
| R6 | IP 越界 | CRITICAL | 复制原文/人物名 | 法律风险 | 只用结构设定；交付物做命名与文本审查 |
...
4. IP 边界沿用 §13：只借鉴设定结构，不复制原文 / 台词 / 人物姓名。
