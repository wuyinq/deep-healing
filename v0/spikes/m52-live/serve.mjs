...
const OUT = resolve(option('out', join(WORKSPACE, 'spikes', 'm52-live', 'readback')));
const RUNTIME = resolve(option('runtime', join(WORKSPACE, 'spikes', 'm52-live', 'runtime', `live-${TAG}`)));
...
const DRIVER = join(HERE, 'tools', 'live_driver.py');
  ...
  if (path === '/live/health' || path === '/live/state' || path === '/live/meta') {
    ...
    if (path === '/live/meta') {
      sendJson(response, 200, { pack_id: PACK, seed: SEED, pace_s_per_tick: 1.0,
                                ...
                                provided_by: 'spikes/m52-live/serve.mjs (LiveWorld projection)' });
      return;
    }
...
http.on('upgrade', (request, socket, head) => {
  ...
  wss.handleUpgrade(request, socket, head, (ws) => {
    ...
    const backlog = state.eventBacklog.slice(-BACKLOG_CAP);
...
writeFileSync(join(OUT, `ac5-run-${TAG}.json`), `${JSON.stringify({
  ...
  tick_series_file: `spikes/m52-live/readback/tick-series-${TAG}.json`,
  events_file: `spikes/m52-live/readback/events-${TAG}.jsonl`,
