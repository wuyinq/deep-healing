...
| R6 | IP 越界 | CRITICAL | 逐字搬运原著正文段落由 `verbatim_body_paragraph`（**不可豁免**）机械判红；原著人物 / 名称 / 情节属**核心依据**（保真度方向，出处表见 `02_source/fidelity/`） | 逐字搬运原著正文段落 | 法律风险 | 内容包 `pack.json.license=proprietary-self-authored`；素材自研/程序化；机制词命中由 `scan_fidelity_terms.py` 判（pack 面 M5.2 启用） |
