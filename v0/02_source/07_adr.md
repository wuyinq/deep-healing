...
## ADR-016 世界观作为可检查契约（治愈内核 + 悬疑外壳）

- 状态：已采纳（M3）
- 依据：`REQ-20260921-005` §4 AC-M3-8、`ISS-20260922-001`、设计输入 `worldview.md`
- 背景：世界观在架构冻结轮是「设计输入」，但在 M1/M2 需求书里没有变成验收判据 ⇒ 实现层只呈现了「治愈」，
  「悬疑外壳」在交接中静默蒸发（ISS-20260922-001）。
- 决定：
  1. 每个 pack **必须**携带 `worldview.json`（经 `pack.entrypoints.worldview` 登记、过 `worldview.schema.json`）；
     基调写成**可检查数值**（`light_k` / `ambient_ratio` / `saturation_pct`），**禁止形容词**。
  2. 「同一几何两态」是硬约束：表层与深层**共用同一份几何数据**（顶点数与实体 id 集合逐项相同），
     深层态 = **同一色板降明度**（不升饱和）；**禁止**第二张地图、第二个 pack、换相机。
  3. 异常锚点 `anomalies[]` 带 `reveal_at_tick`，其「表层读法 / 深层读法」两段必须**可核验**；
     可介入的系统性故障写进 `fault_lines[]`，与任务状态机（`task.state_changed`）数据驱动联动。
  4. 每条落地判据**必须带自证反例**（把行为退回修复前 ⇒ 判据必须红）。
- 后果与迁移：
  - `entrypoints` 由 6 键扩为 7 键 ⇒ `district.pack.schema.json` 同步更新，**两个 pack 重签 `pack.sig`**；
